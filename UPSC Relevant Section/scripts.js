        ...
        function renderArticles(dataKey) {
            ...
            // 1. GET SAVED STATUS FROM STORAGE
            const savedList = JSON.parse(localStorage.getItem('mindMapData') || '[]');
            ...
            data.articles.forEach((article, index) => {
                ...
                // 2. CHECK IF THIS ARTICLE IS ALREADY SAVED
                // We check if any item in the saved list has the same title
                const isSaved = savedList.some(item => item.title === article.title);
                const activeClass = isSaved ? 'active' : ''; // Add 'active' class if found
                ...
                const articleHTML = `
                    <div class="article-card">
                        ...
                        <div class="content-block">
                            <span class="content-label">The Context</span>
                            <p style="margin:0;">${article.context}</p>
                        </div>
                        
                        <div class="content-block">
                            <span class="content-label">Key Arguments</span>
                        ...
                        <div class="grid-2-col">
                            <div class="info-box box-green">
                                <span class="box-label">Way Forward</span>
                                <ul class="no-bullet-list">
                                    ${article.wayForward.map(pt => `<li><span class="bullet-icon">•</span><span>${pt}</span></li>`).join('')}
                                </ul>
                            </div>
                            <div class="info-box box-amber">
                                <span class="box-label">Mains Fodder</span>
                                <div>
                                    ${Object.entries(article.fodder).map(([key, val]) => 
                                        `<div class="key-val-row"><span class="key-label">${key}:</span><span>${val}</span></div>`
                                    ).join('')}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-actions">
                            <!-- 3. APPLY THE ACTIVE CLASS HERE -->
                            <button class="btn-save ${activeClass}" onclick="toggleSave(this, '${safeTitle}', '${article.originalLink}', 'UPSC', '${article.context.replace(/'/g, "\\'")}')" title="Save for Dashboard">
                            ...
                            <a href="${article.wikiLink}" target="_blank" class="btn-wiki" title="Refer to Wikipedia">
                            ...
                            <div style="display:flex; gap:8px;">
                                ...
                                <a href="${searchUrl}" target="_blank" class="btn-outline" title="Search Fallback" style="padding: 10px;">
